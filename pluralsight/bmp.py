...
def write_grayscale(filename, pixels):
    ...
    Args:
        ...
        pixels: A rectangular image stored as a sequence of rows.
            Each row must be an iterable series of integers in range 0-255
    ...
    with open(filename, mode='wb') as bmp:
        ...
        bmp.write(b'\x00\x00\x00\x00')  # Unused pixels per metter
        bmp.write(b'\x00\x00\x00\x00')  # Unused pixels per metter
...
def _int32_to_bytes(integer):
    '''Convert an integer to four bytes in little-endian format.'''
    return bytes((integer & 0xff,
                  integer >> 8 & 0xff,
                  integer >> 16 & 0xff,
                  integer >> 24 & 0xff))

def _bytes_to_int32(b):
    '''Convert a 4 bytes typle to integer.'''
    return b[0] | (b[1] << 8) | (b[2] << 16) | (b[3] << 24)

def dimensions(filename):
    '''Retrieves dimansion of BPM image.'''
